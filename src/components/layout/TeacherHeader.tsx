...
import { LogOut, ClipboardList, FileBarChart2 } from 'lucide-react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';

interface HeaderProps {
  title: string;
  activePage: 'attendance' | 'test-report';
}
...
export const TeacherHeader = ({ title, activePage }: HeaderProps) => {
  ...
  // Reusable Nav Item Component with Tooltip Logic
  ...
  return (
    <header className="bg-white border-b border-gray-100 px-4 md:px-8 py-3 md:py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <nav className={`
            ...
            /* Mobile Positioning: Floating below header */
        ...
        <div className="flex items-center gap-3 md:gap-6">
           ...
           <button 
             ...
             className="p-2.5 bg-red-50 hover:bg-[#B70003] text-red-600 hover:text-white rounded-xl transition-all shadow-sm cursor-pointer group active:scale-90"
             title="Logout"
           >
