...
const StatCard = ({ label, value, icon: Icon, color, delay, onClick, active }: any) => (
    <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }}
        onClick={onClick}
        className={`bg-white p-6 rounded-[20px] shadow-sm border ${active ? 'border-[#B50104] ring-2 ring-[#B50104]/20' : 'border-gray-100'} flex items-center gap-4 hover:shadow-md transition-all cursor-pointer`}
    >
...
export const ParentsOverview = () => {
    ...
    const filteredParents = parents.filter(p => {
        const matchesSearch = p.fatherName.toLowerCase().includes(search.toLowerCase()) ||
            p.mobileNo.includes(search) ||
            p.cnic?.includes(search);

        let matchesStatus = true;
        if (filterStatus === 'Active') matchesStatus = p.status === 'Active';
        if (filterStatus === 'Inactive') matchesStatus = p.status !== 'Active';
        if (filterStatus === 'Missing CNIC') matchesStatus = !p.cnic;
    ...
    return (
        <div className="space-y-8 font-['Montserrat'] animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard label="Total Parents" value={parents.length} icon={Users} color="bg-[#B50104]" delay={0} onClick={() => setFilterStatus("All")} active={filterStatus === "All"} />
                <StatCard label="Active Parents" value={parents.filter(p => p.status === 'Active').length} icon={UserCheck} color="bg-[#009952]" delay={0.1} onClick={() => setFilterStatus("Active")} active={filterStatus === "Active"} />
                <StatCard label="Inactive" value={parents.filter(p => p.status !== 'Active').length} icon={UserX} color="bg-gray-400" delay={0.2} onClick={() => setFilterStatus("Inactive")} active={filterStatus === "Inactive"} />
                <StatCard label="Missing CNIC" value={parents.filter(p => !p.cnic).length} icon={AlertTriangle} color="bg-yellow-500" delay={0.3} onClick={() => setFilterStatus("Missing CNIC")} active={filterStatus === "Missing CNIC"} />
            </div>
