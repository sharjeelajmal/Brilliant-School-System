...
function DashboardContent() {
  ...
  useEffect(() => { setActivePage(tab); }, [tab]);
    ...
    <div className="flex min-h-screen bg-[#FDFDFD] font-['Montserrat'] overflow-hidden">
      ...
      <main className="flex-1 p-8 md:p-12 h-screen overflow-y-auto">
        ...
        <AnimatePresence mode="wait">
          {activePage === 'overview' ? <Overview key="overview" /> :
           activePage === 'classes' ? <motion.div key="classes" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}><ClassesOverview /></motion.div> :
           activePage === 'forms' ? <motion.div key="forms" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}><AdmissionForm /></motion.div> :
           activePage === 'teachers' ? <motion.div key="teachers" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}><TeacherHiringForm /></motion.div> :
           activePage === 'complaints' ? <motion.div key="complaints" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}><ComplaintManager /></motion.div> :
           activePage === 'students' ? <motion.div key="students" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><StudentsOverview onNavigate={handlePageChange} /></motion.div> :
           ...
           activePage === 'test-report' ? (
               <motion.div key="test-report" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                   <TestReportContent />
               </motion.div>
           ) : (
